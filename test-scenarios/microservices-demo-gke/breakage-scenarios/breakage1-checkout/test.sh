#!/bin/bash
echo "🔍 Continually pinging Scenario 1 status (via simulated checkout request, Ctrl+C to stop)..."
...
while true; do
  ...
  if [ "$HTTP_CODE" = "200" ]; then
    ...
    echo "$(date '+%H:%M:%S') 🔴 [scenario1] BLOCKED: NetworkPolicy is blocking checkout traffic! (HTTP $HTTP_CODE)"
  fi
  sleep 1.5
done
